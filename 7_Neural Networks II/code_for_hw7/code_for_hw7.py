...
class Linear(Module):
    ...
    def backward(self, dLdZ):  # dLdZ is (n x b), uses stored self.A
        ...
        # dLdW0 is surprisingly sum of all dLdZ of each datapoint(or batch), keeping dimension n x 1
...
class Tanh(Module):  # Layer activation
    ...
    def backward(self, dLdA):  # Uses stored self.A
        v = 1 - np.square(self.A)   # Rank 1 view
        d, n = v.shape
        # initializing a tensor to store diagonal matrices for all data points
        dAdZ = np.zeros((n, d, d))
        # initializing dLdZ
        dLdZ = np.zeros((d, n))
        # Filling up dAdZ
        for i in range(n):
            dAdZ[i, :, :] = np.diag(v[:, i])
        # multiplying respective diag matrices with corresponding columns i.e. datapoints of dLdA
        for i in range(n):
            dLdZ[:, i] = np.dot(dAdZ[i, :, :], dLdA[:, i])
        return dLdZ  # Your code: return dLdZ (?, b)


class ReLU(Module):  # Layer activation
    def forward(self, Z):
        self.A = self.A = Z * (Z > 0) #Your code: (?, b)
        return self.A

    def backward(self, dLdA):  # uses stored self.A
        v = 1 * (self.A > 0)  # Rank 1 view, done to make a valid input for np.diag
        d,n = v.shape
        # initializing a tensor to store diagonal matrices for all data points
        dAdZ = np.zeros((n, d, d))
        # initializing dLdZ
...
class NLL(Module):  # Loss
    def forward(self, Ypred, Y):
        ...
        return np.sum(nll) / self.Ypred.shape[0] # Your code: return loss (scalar)

    def backward(self):  # Use stored self.Ypred, self.Y
        # refer to softmax derviative and cross entropy loss for towardsdatascience.com
