...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    # Finding hyperplane parameter for training data
    th, th0 = learner(data_train, labels_train, {'T':100})
    # Evaluating the classifier on a testing dataset
    d, n = data_test.shape
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    corrects = 0
    for i in range (it):
        # Generating training data of specified size
        train_data, train_label = data_gen(n_train)
        # Generating testing data set
        test_data, test_label = data_gen(n_test)
        # Evluating classifier
        correct = eval_classifier(learner, train_data, train_label, test_data, test_label)
        # Total average correct predictions upto now
        corrects += correct
    return corrects/it
...
def xval_learning_alg(learner, data, labels, k):
    # Dividing the data and labels into k equal parts
    D = np.array_split(data, k, axis=1)
    L = np.array_split(labels, k, axis=1)
    scores = []

    for j in range(k):
        # Combining all the array section other than the jth section
        D_minus_j = np.concatenate((D[ :j] + D[j+1: ]), axis=1)
        L_minus_j = np.concatenate((L[ :j] + L[j+1: ]), axis=1)
        # Evaluating the classifier: trained on D_minu_j and tested on D[j]
        correct = eval_classifier(learner, D_minus_j, L_minus_j, D[j], L[j])
...
print(eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=0.1), 20, 20, 300))
